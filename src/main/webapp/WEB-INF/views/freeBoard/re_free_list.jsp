        ...
        <ul class="pagination">
            <c:if test="${pp.startPage > pp.pagePerBlk }">
                <li class="page-item"><a
                        href="r_view?rpageNum=${pp.startPage - 1}&num=${num}&board_num=${board_num}">이전</a></li>
            </c:if>

            <c:forEach var="i" begin="${pp.startPage}" end="${pp.endPage}">
                <c:if test="${pp.currentPage==i}">
                    <li class="page-item active" aria-current="page">
                    <a class="page-link" href="r_view?rpageNum=${i}&num=${num}&board_num=${board_num}">${i}</a>
                </c:if>
            ...
            <c:if test="${pp.endPage < pp.totalPage}">
                <li><a href="r_view?rpageNum=${pp.endPage + 1}&num=${num}&board_num=${board_num}">다음</a></li>
            </c:if>
        </ul>
    </nav>
</div>
